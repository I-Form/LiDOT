function write3mf(filename , vertices , faces, beams, colors,Diam)
...
%     checkInput(filename , vertices , faces, colors);
    ...
    % 3D model
    write3DModel(files , vertices , faces, beams, colors,Diam);
...
% reproducing the structure of the 3mf file. 
    ...
    try
        ...
        files.temppath = [mfilename('fullpath') , '_temp6/'];
...
function write3DModel(files , vertices , faces, beams, colors,Diam)
% Writes the 3D model proper
    ...
    % Write header
    fprintf(fid , '<?xml version="1.0" encoding="UTF-8"?>\n');
    fprintf(fid , '<model unit="millimeter" xml:lang="en-US" xmlns:m="http://schemas.microsoft.com/3dmanufacturing/material/2015/02" xmlns:b="http://schemas.microsoft.com/3dmanufacturing/beamlattice/2017/02" xmlns="http://schemas.microsoft.com/3dmanufacturing/core/2015/02">\n');
    ...
    for k = 1 : size(vertices , 1)
        fprintf(fid , '\t\t\t\t\t<vertex x="%.6f" y="%.6f" z="%.6f" />\n' , vertices(k,:));        
    end    
    fprintf(fid , '\t\t\t\t</vertices>\n');
    ...
    if size(beams, 1) ~= 1
    % Beam Lattices
        fprintf(fid ,'\t\t\t\t<b:beamlattice radius="0.1" minlength="0.0000001" cap="sphere">\n');
        fprintf(fid , '\t\t\t\t\t<b:beams>\n');
        if isempty(colors)
            for k = 1 : size(beams , 1)
                    fprintf(fid , '\t\t\t\t\t\t<b:beam v1="%u" v2="%u" r1="%6.4f" r2="%6.4f"/>\n' , [beams(k,[1 2])-1 Diam(k,1)/2 Diam(k,2)/2]);
            end       
        end
        fprintf(fid , '\t\t\t\t\t</b:beams>\n');
        fprintf(fid , '\t\t\t\t</b:beamlattice>\n');
    end
